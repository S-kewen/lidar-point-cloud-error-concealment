...
def eval_one_epoch(model, eval_loader, epoch, tb_log=None, log_f=None):
    ...
    for it, batch in enumerate(eval_loader):
        ...
        numpy_pred_class = pred_class.cpu().numpy().reshape([-1, 1])

        _ = np.concatenate((batch['original_input'].reshape((-1, 4)), numpy_pred_class), axis=1)
        point_xyzis_gt = np.concatenate((batch['original_input'].reshape((-1, 4)), _cls_labels.reshape((-1, 1))), axis=1)
        fg_points_xyzis = _[np.where(_[:, 4] > 0), :4].reshape([-1, 4])
        fg_points_xyzis_gt = point_xyzis_gt[np.where(_[:, 4] > 0), :4].reshape([-1, 4])

        save_name = Path() / "output" / "bin" / "{0:06}.bin".format(batch['sample_id'][0])
        save_name_original = Path() / "output" / "bin" / "{0:06}_original.bin".format(batch['sample_id'][0])
        batch['fg_points_gt'].astype(np.float32).tofile(str(save_name))
        batch['original_input'].astype(np.float32).tofile(str(save_name_original))
