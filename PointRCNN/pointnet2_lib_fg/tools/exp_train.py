    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.ckpt_save_interval == 0:
            ...
            with torch.no_grad():
                ...
                print("epoch {}, avg_iou: {}".format(epoch, avg_iou))
...
def main():
    ...
    eval_path = Path(m_config["exp"]["eval_path"]).parent.parent
    print("train_path: {}".format(train_path))
    print("eval_path: {}".format(eval_path))
    
    train_set = KittiDataset(root_dir=train_path, mode='TRAIN', split='train', lidar_dir="velodyne")
    train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, pin_memory=True,
                              num_workers=args.workers, collate_fn=train_set.collate_batch)
    
    
    eval_set = KittiDataset(root_dir=eval_path, mode='EVAL', split='train', lidar_dir="velodyne")
