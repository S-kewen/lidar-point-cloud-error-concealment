...
with open(Path().cwd().parent.parent / 'config.yaml', 'r') as yamlfile:
...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    if cfg.TEST.SPLIT != 'test':
        logger.info('!!! Averate Precision:')
        name_to_class = {'Car': 0, 'Pedestrian': 1, 'Cyclist': 2}
        ap_result_str, ap_dict = kitti_evaluate(dataset.label_dir, 
                                                final_output_dir, 
...
def eval_single_ckpt(root_result_dir, dataset_path):
    ...
    test_loader = create_dataloader(logger, dataset_path)
...
def repeat_eval_ckpt(root_result_dir, ckpt_dir, dataset_path):
...
def create_dataloader(logger, dataset_path = os.path.join('..', 'data')):
    mode = 'TEST' if args.test else 'EVAL'
...
if __name__ == "__main__":
    ...
    # dataset_path = os.path.join('/', 'mnt', 'sda', "PointRCNN", 'carla')
    # dataset_path = cfg.DATASET_PATH
    
    dataset_path = Path(m_config["exp"]["train_path"]).parent.parent
    
    with torch.no_grad():
        if args.eval_all:
            assert os.path.exists(ckpt_dir), '%s' % ckpt_dir
            repeat_eval_ckpt(root_result_dir, ckpt_dir, dataset_path)
        else:
            eval_single_ckpt(root_result_dir, dataset_path)