...
def create_dataloader(logger, DATA_PATH = os.path.join('../', 'data')):
...
def random_split_train_val(file_name, save_name, val_ratio=0.1):
    import random
    
    with open(file_name, 'r') as f:
        data = f.readlines()
        sample_num = int(len(data)*val_ratio)
        sample_num = sample_num if sample_num > 0 else 1
        val_data = random.sample(data, sample_num)
...
if __name__ == "__main__":
    ...
    train_path = Path(m_config["exp"]["train_path"]).parent.parent
    print("train_path: {}".format(train_path))
    
    if args.train_with_eval:
        random_split_train_val(train_path / "ImageSets/train.txt", train_path / "ImageSets/{}.txt".format(cfg.TRAIN.VAL_SPLIT), 0.1)
        
    train_loader, test_loader = create_dataloader(logger, train_path)
