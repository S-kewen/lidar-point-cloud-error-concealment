...
parser.add_argument("--c", type=str, default="config.yaml", help="specify config file")
...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    dataset = dataloader.dataset
    save_dir = "{}/evaluation".format(dataset.lidar_dir)
    ...
    progress_bar = tqdm.tqdm(total=len(dataloader), leave=True, desc='eval')
    for data in dataloader:
        #print("dataloader: {}".format(data))
        cnt += 1
        sample_id, pts_rect, pts_features, pts_input = \
            data['sample_id'], data['pts_rect'], data['pts_features'], data['pts_input']
        
        
        if pts_input.shape[1] <= 0:
            continue
        # if pts_input.shape[1] <= 0:
        #     #np.zeros((2, 7, 3))
        #     save_metrics(save_dir, [np.zeros((7, 3)), np.zeros((7, 3))])
        #     return None
        
        batch_size = len(sample_id)
        inputs = torch.from_numpy(pts_input).cuda(non_blocking=True).float()
        #print("inputs: {}".format(pts_input.shape[1]))
        input_data = {'pts_input': inputs}
        ...
        if not args.test:
            ...
            for k in range(batch_size):
                ...
                if tmp_idx >= 0:
                    ...
                    iou3d_np = iou3d.cpu().numpy()
                    iou3d_np = iou3d_np.reshape((-1))
                    # logger.info("iou3d: {}".format(iou3d_np))
                    avg_iou_list.append(iou3d_np)
                    gt_max_iou, _ = iou3d.max(dim=0)
                    refined_iou, _ = iou3d.max(dim=1)
        ...
        # for k in range(batch_size):
        #     iou3d = iou3d_utils.boxes_iou3d_gpu(pred_boxes3d[k], gt_boxes3d[k])
        #     avg_iou_list.append(iou3d)
        #     logger.info("iou3d: {}".format(iou3d.shape))
    ...
    if cfg.TEST.SPLIT != 'test':
        ...
        logger.info('lidar path: {}'.format(dataset.lidar_dir))
        name_to_class = {'Car': 0, 'Pedestrian': 1, 'Cyclist': 2}
        ap_result_str, ap_dict, result_list = kitti_evaluate(dataset.label_dir, final_output_dir, label_split_file=split_file, current_class=name_to_class[cfg.CLASSES])
        print(ap_result_str)
        # [skewen]: here going to cal the avg iou for threadhold 0.5 and 0.7
        avg_iou_np = np.concatenate(avg_iou_list, axis=0)
        for t in [0.5, 0.7]:
            avg_iou = avg_iou_np[avg_iou_np >= t]
            np.save("{}/obj_pointrcnn_car_{}_avg_iou.npy".format(save_dir, t), [[0, np.mean(avg_iou)]])
            print("t:{}, avg_iou: {}".format(t, np.mean(avg_iou)))
        ret_dict.update(ap_dict)
        save_metrics(save_dir, result_list)
   

    logger.info('result is saved to: %s' % result_dir)
    return ret_dict

def get_avg_iou(parted_overlaps, threadhold):
    print("parted_overlaps.shape: ", parted_overlaps.shape)
    return np.mean(parted_overlaps >= threadhold)



def save_metrics(save_dir, result_list):
    # Car AP@0.70, 0.70, 0.70:
    # bbox AP:90.7236, 90.7273, 90.7273
    # bev  AP:7.3926, 7.3926, 7.3926
    # 3d   AP:1.8182, 1.8182, 1.8182
    # aos  AP:74.06, 73.99, 73.99
    # Car AP@0.70, 0.50, 0.50:
    # bbox AP:90.7236, 90.7273, 90.7273
    # bev  AP:90.7197, 90.7236, 90.7236
    # 3d   AP:77.7474, 77.4753, 77.4753
    # aos  AP:74.06, 73.99, 73.99

    # result_list: [[0, 'Car', array([0.7, 0.7, 0.7]), array([90.72356215, 90.72727273, 90.72727273]), array([7.39260739, 7.39260739, 7.39260739]), array([1.81818182, 1.81818182, 1.81818182]), array([74.06429229, 73.98802521, 73.98802521])], [0, 'Car', array([0.7, 0.5, 0.5]), array([90.72356215, 90.72727273, 90.72727273]), array([90.71969697, 90.72356215, 90.72356215]), array([77.74740411, 77.47529527, 77.47529527]), array([74.06429229, 73.98802521, 73.98802521])]]
    
    np.save("{}/obj_pointrcnn_car_0.7.npy".format(save_dir), result_list[0])
    np.save("{}/obj_pointrcnn_car_0.5.npy".format(save_dir), result_list[1])
    
    # np.save("{}/obj_pointrcnn_car_easy_aos.npy".format(save_dir), [[0, result_list[0][6][0]]])
    # np.save("{}/obj_pointrcnn_car_mod_aos.npy".format(save_dir), [[0, result_list[0][6][1]]])
    # np.save("{}/obj_pointrcnn_car_hard_aos.npy".format(save_dir), [[0, result_list[0][6][2]]])
    #print("car_aos: {}".format(result_list[0][6]))
    # START SAVING Car AP@0.7 0.7 0.7
    np.save("{}/obj_pointrcnn_car_0.7_easy_bbox.npy".format(save_dir), [[0, result_list[0][3][0]]])
    np.save("{}/obj_pointrcnn_car_0.7_easy_bev.npy".format(save_dir), [[0, result_list[0][4][0]]])
    np.save("{}/obj_pointrcnn_car_0.7_easy_3d.npy".format(save_dir), [[0, result_list[0][5][0]]])
    #print("car_0.7_easy: {}, {}, {}".format(result_list[0][3][0], result_list[0][4][0], result_list[0][5][0]))
    
    np.save("{}/obj_pointrcnn_car_0.7_mod_bbox.npy".format(save_dir), [[0, result_list[0][3][1]]])
    np.save("{}/obj_pointrcnn_car_0.7_mod_bev.npy".format(save_dir), [[0, result_list[0][4][1]]])
    np.save("{}/obj_pointrcnn_car_0.7_mod_3d.npy".format(save_dir), [[0, result_list[0][5][1]]])
    #print("car_0.7_mod: {}, {}, {}".format(result_list[0][3][1], result_list[0][4][1], result_list[0][5][1]))
    
    np.save("{}/obj_pointrcnn_car_0.7_hard_bbox.npy".format(save_dir), [[0, result_list[0][3][2]]])
    np.save("{}/obj_pointrcnn_car_0.7_hard_bev.npy".format(save_dir), [[0, result_list[0][4][2]]])
    np.save("{}/obj_pointrcnn_car_0.7_hard_3d.npy".format(save_dir), [[0, result_list[0][5][2]]])
    #print("car_0.7_hard: {}, {}, {}".format(result_list[0][3][2], result_list[0][4][2], result_list[0][5][2]))
    
    # START SAVING Car AP@0.7 0.5 0.5
    np.save("{}/obj_pointrcnn_car_0.5_easy_bev.npy".format(save_dir), [[0, result_list[1][4][0]]])
    np.save("{}/obj_pointrcnn_car_0.5_easy_3d.npy".format(save_dir), [[0, result_list[1][5][0]]])
    #print("car_0.5_easy: {}, {}, {}".format(result_list[1][3][0], result_list[1][4][0], result_list[1][5][0]))
    
    np.save("{}/obj_pointrcnn_car_0.5_mod_bev.npy".format(save_dir), [[0, result_list[1][4][1]]])
    np.save("{}/obj_pointrcnn_car_0.5_mod_3d.npy".format(save_dir), [[0, result_list[1][5][1]]])
    #print("car_0.5_mod: {}, {}, {}".format(result_list[1][3][1], result_list[1][4][1], result_list[1][5][1]))
    
    np.save("{}/obj_pointrcnn_car_0.5_hard_bev.npy".format(save_dir), [[0, result_list[1][4][2]]])
    np.save("{}/obj_pointrcnn_car_0.5_hard_3d.npy".format(save_dir), [[0, result_list[1][5][2]]])
    #print("car_0.5_hard: {}, {}, {}".format(result_list[1][3][2], result_list[1][4][2], result_list[1][5][2]))
    
def eval_one_epoch(model, dataloader, epoch_id, result_dir, logger):
    if cfg.RPN.ENABLED and not cfg.RCNN.ENABLED:
        print("eval_one_epoch RPN")
        ret_dict = eval_one_epoch_rpn(model, dataloader, epoch_id, result_dir, logger)
    elif not cfg.RPN.ENABLED and cfg.RCNN.ENABLED:
        print("eval_one_epoch RCNN")
        ret_dict = eval_one_epoch_rcnn(model, dataloader, epoch_id, result_dir, logger)
    elif cfg.RPN.ENABLED and cfg.RCNN.ENABLED:
        print("eval_one_epoch RPN && RCNN")
...
def load_ckpt_based_on_args(model, logger):
    ...
    if cfg.RCNN.ENABLED and args.rcnn_ckpt is not None:
        print("load_part_ckpt: RCNN")
        load_part_ckpt(model, filename=args.rcnn_ckpt, logger=logger, total_keys=total_keys)


def eval_single_ckpt(root_path, root_result_dir, dataset_path, lidar_dir = "velodyne"):
    ...
    save_split_txt(root_path / lidar_dir, image_sets_dir / "{}.txt".format(image_sets_split))
    
    test_loader = create_dataloader(root_path, logger, dataset_path, lidar_dir, image_sets_split)
...
def repeat_eval_ckpt(root_path, root_result_dir, ckpt_dir, dataset_path, lidar_dir):
    ...
    test_loader = create_dataloader(root_path, logger, dataset_path)
    model = PointRCNN(num_classes=test_loader.dataset.num_class, use_xyz=True, mode='TEST')
    model.cuda()
...
def create_dataloader(root_path, logger, dataset_path = os.path.join('..', 'data'), lidar_dir="velodyne", split = cfg.TEST.SPLIT):
    mode = 'TEST' if args.test else 'EVAL'
    DATA_PATH = dataset_path

    # create dataloader
    test_set = KittiRCNNDataset(root_dir=DATA_PATH, npoints=cfg.RPN.NUM_POINTS, split=split, mode=mode,
                                random_select=args.random_select,
                                rcnn_eval_roi_dir=args.rcnn_eval_roi_dir,
                                rcnn_eval_feature_dir=args.rcnn_eval_feature_dir,
                                classes=cfg.CLASSES,
                                logger=logger, lidar_dir=lidar_dir, source_path=root_path)
...
def main(root_path, dataset_path, index, lidar_dir):
    ...
    if args.eval_mode == 'rpn':
        ...
        root_result_dir = os.path.join('../', 'output', timestamp, 'rpn', cfg.TAG)
        ckpt_dir = os.path.join('../', 'output', timestamp, 'rpn', cfg.TAG, 'ckpt')
    elif args.eval_mode == 'rcnn':
        cfg.RCNN.ENABLED = True
        cfg.RPN.ENABLED = cfg.RPN.FIXED = True
        root_result_dir = os.path.join('../', 'output', timestamp, 'rcnn', cfg.TAG)
        ckpt_dir = os.path.join('../', 'output', timestamp, 'rcnn', cfg.TAG, 'ckpt')
    ...
    print("[{}] start eval: {}".format(index, lidar_dir))
    with torch.no_grad():
        if args.eval_all:
            assert os.path.exists(ckpt_dir), '%s' % ckpt_dir
            repeat_eval_ckpt(root_path, root_result_dir, ckpt_dir, dataset_path, lidar_dir)
        else:
            eval_single_ckpt(root_path, root_result_dir, dataset_path, lidar_dir)
...
if __name__ == "__main__":
    with open(Path().cwd().parent.parent / args.c, 'r') as yamlfile:
        m_config = yaml.load(yamlfile)
    start_time = time.time()
    
    root_path = Path(m_config["exp"]["path"])
    
    dataset_path = root_path.parent.parent
    print("dataset path: {}".format(dataset_path))
    
    lidar_dirs = ["velodyne","velodyne_compression"]#, "cache_frame/adaptive_sampling"

    base_path = Path() / m_config["exp"]["path"] / "cache_frame"
    for i in base_path.iterdir():
        lidar_dirs.append("cache_frame/{}".format(i.name))
    
    for i, lidar_dir in enumerate(lidar_dirs):
        main(root_path, dataset_path, i, lidar_dir)
...
#             root_path = Path() / "/mnt/data2/skewen/kittiGenerator/output" / dataset / "object" / agent
...
#             lidar_dirs = ["velodyne"]#, "cache_frame/adaptive_sampling"
