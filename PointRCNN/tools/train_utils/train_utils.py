        ...
        with tqdm.trange(start_epoch, n_epochs, desc='epochs') as tbar, \
            ...
            for epoch in tbar:
                ...
                if (epoch % eval_frequency) == 0:
                    cur_logger.info("start eval epoch {}...".format(trained_epoch))
                    pbar.close()
                    if test_loader is not None:
                        with torch.set_grad_enabled(False):
                            val_loss, eval_dict, cur_performance = self.eval_epoch(test_loader)
                        cur_logger.info("[{}] val_loss: {}, eval_dict: {}, cur_performance: {}".format(trained_epoch, val_loss, eval_dict, cur_performance))
                        if self.tb_log is not None:
                            self.tb_log.add_scalar('val_loss', val_loss, it)
                            self.tb_log.add_scalar('cur_performance', cur_performance, it)
