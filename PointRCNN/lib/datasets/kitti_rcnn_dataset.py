...
class KittiRCNNDataset(KittiDataset):
    def __init__(self, root_dir, npoints=16384, split='train', classes='Car', mode='TRAIN', random_select=True,
                 ...
                 rcnn_eval_feature_dir=None, gt_database_dir=None, lidar_dir="velodyne", source_path=None):
        super().__init__(root_dir=root_dir, split=split, lidar_dir = lidar_dir, source_path=source_path)
        if classes == 'Car':
            self.classes = ('Background', 'Car')
            aug_scene_root_dir = os.path.join(root_dir,  'aug_scene')
        elif classes == 'People':
            self.classes = ('Background', 'Pedestrian', 'Cyclist')
        elif classes == 'Pedestrian':
            self.classes = ('Background', 'Pedestrian')
            aug_scene_root_dir = os.path.join(root_dir,  'aug_scene_ped')
        elif classes == 'Cyclist':
            self.classes = ('Background', 'Cyclist')
            aug_scene_root_dir = os.path.join(root_dir,  'aug_scene_cyclist')
        else:
    ...
    def get_valid_flag(pts_rect, pts_img, pts_rect_depth, img_shape):
        ...
        # [skewen]: choose the 360 argee points
        # return np.full(pts_rect.shape[0], True, dtype=np.bool)
            ...
            if self.npoints < len(pts_rect):
                ...
                if len(choice) > 0:
                    ...
                    if self.npoints > len(pts_rect):
                        ...
                        extra_choice = np.random.choice(choice, self.npoints - len(pts_rect), replace=(self.npoints - len(pts_rect))>len(choice))
