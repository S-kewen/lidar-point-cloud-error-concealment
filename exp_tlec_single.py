...
def fun_threshold_based_lidar_error_concealment(exp_path, packet_loss_rate, pro_name, args, threadhold_tp, threadhold_tn):
    ...
    for i, file_name in tqdm(enumerate(file_names), desc="{} {} ({}, {})".format(pro_name, packet_loss_rate, threadhold_tp, threadhold_tn), total=len(file_names)):
...
def main(args):
    ...
    assert args.tp > 0 and args.tn > 0, "tp and tn error"

    if m_config["exp"][execution_name]["enable"]:
        for packet_loss_rate in packet_loss_rates:
            globals()["fun_{}".format(execution_name)](exp_path, packet_loss_rate, execution_name, copy.deepcopy(m_config["exp"][execution_name]), args.tp, args.tn)
