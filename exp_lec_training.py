...
from exp_generating import format_ply_random_by_packet_loss_rate_xyzi
from eval_util import EvalUtil as eval_util
import random



def fun_generation(file_name, packet_loss_rate, save_dir):
...
def fun_lidar_error_concealment_training(exp_path, pro_name, args):
    ...
    none_dir = Path() / exp_path / "cache_frame" / "receiver_none_random"
    my_util.create_dir(none_dir)
    
    # generate incomplete frames
    file_names = sorted(velodyne_compression_dir.glob("*.bin"))
    for i, file_name in tqdm(enumerate(file_names), desc="generation random {}".format(pro_name), total=len(file_names)):
        packet_loss_rate = round(random.random(), 2)
        fun_generation(file_name, packet_loss_rate, none_dir)
    
    file_names = sorted(none_dir.glob("*.bin"))
    file_count = len(file_names)
    assert file_count > 0, "bin files not exists: {}".format(none_dir)
    
    save_dir = Path() / exp_path / "cache_frame" / "receiver_lidar_error_concealment_training"
    my_util.create_dir(save_dir)
    
    first_file_name = velodyne_compression_dir / "{}.bin".format(file_names[0].stem)
    save_first_frame(first_file_name, point_count, save_dir, sector_size)
    
    result_list = []
    
    save_name = "model_{}".format(pro_name)
    
    for i, file_name in tqdm(enumerate(file_names), desc="{}_training".format(pro_name), total=len(file_names)):
        ...
        if dropped_points_yp.shape[0] > 0 and received_points_ypd.shape[0] > 0:
        ...
        result_list.append([sector_list_rate_p, sector_lost_rate, sector_lost_rate_n, opm_algo_index, tp_cd, si_cd, ti_cd])
        
        # print([tp_cd, si_cd, ti_cd, opm_algo_index])
        ...
        running_time = time.process_time() - start_time
        np.save(save_dir / "{}_log.npy".format(file_name.stem), {"id": i + 1, "file_name": str(file_name), "concealment_types": opm_algo_index, "sector_size": sector_size, "sector_list": sector_list + drop_sector_list, "concealed_sector_list": drop_sector_list, "time": time.time(), "running_time": running_time})
    
        write_text(Path() / exp_path / "{}.csv".format(save_name), "{},{},{},{},{},{},{}".format(sector_list_rate_p, sector_lost_rate, sector_lost_rate_n, opm_algo_index, tp_cd, si_cd, ti_cd))
    np.save(Path() / exp_path / "{}.npy".format(save_name), np.array(result_list))

def main(args):
    m_config = my_util.get_config(args.c)
    execution_name = "lidar_error_concealment"
    exp_path = m_config["exp"]["path"]
    fun_lidar_error_concealment_training(exp_path, execution_name, copy.deepcopy(m_config["exp"][execution_name]))
