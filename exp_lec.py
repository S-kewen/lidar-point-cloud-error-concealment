...
def fun_lidar_error_concealment(exp_path, packet_loss_rate, pro_name, args, model_type = "decision_tree"):
    ...
    # init ml model
    decision_model_path = args["decision_model_path"]
    training_set = np.load(decision_model_path, allow_pickle=True) # loading training set
    
    X = np.round(training_set[:, :3], 2)
    Y = training_set[:, 3]
    
    
    if model_type == "decision_tree":
        pd_model = DecisionTreeClassifier(max_depth=10)
        pd_model.fit(X, Y)
    elif model_type == "svm":
        pd_model = SVC(kernel="rbf", C=10)
        pd_model.fit(X, Y)
    elif model_type == "random_forest":
        pd_model = RandomForestClassifier(n_estimators = 100, max_depth=10)
        pd_model.fit(X, Y)
    else:
        assert False, "NOT SUPPORT MODEL TYPE"
    
    
    
    save_dir = Path() / exp_path / "cache_frame" / "receiver_{}_{}_{}".format(pro_name, model_type, packet_loss_rate)
    ...
    for i, file_name in tqdm(enumerate(file_names), desc="{} {} {}".format(pro_name, model_type, packet_loss_rate), total=len(file_names)):
        ...
        opm_algo_index = pd_model.predict(np.asarray([[sector_list_rate_p, sector_lost_rate, sector_lost_rate_n]]))[0]
        ...
        np.save(save_dir / "{}_log.npy".format(file_name.stem), {"id": i + 1, "file_name": str(file_name), "model_type": model_type, "concealment_types": opm_algo_index, "sector_size": sector_size, "sector_list": sector_list + drop_sector_list, "concealed_sector_list": drop_sector_list, "time": time.time(), "running_time": running_time})
...
def main(args):
    ...
    model_type_list = ["decision_tree", "svm", "random_forest"]

    if m_config["exp"][execution_name]["enable"]:
        for packet_loss_rate in packet_loss_rates:
            for model_type in model_type_list:
                globals()["fun_{}".format(execution_name)](exp_path, packet_loss_rate, execution_name, copy.deepcopy(m_config["exp"][execution_name]), model_type)
